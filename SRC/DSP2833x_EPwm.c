...
void InitEPwm4(void)
{
	   ...
	   EPwm4Regs.TBCTL.bit.PRDLD = TB_IMMEDIATE; // Count up
	   EPwm4Regs.TBPRD = 0;                        // Set timer period
	   EPwm4Regs.CMPA.half.CMPA = 0; //A路占空比
	   EPwm4Regs.CMPB = 0;//B路占空比

	   EPwm4Regs.TBPHS.all=0;
	   EPwm4Regs.TBCTR=0;  //时基计数为0
	   // Setup TBCLK
	   EPwm4Regs.TBCTL.bit.CTRMODE = TB_COUNT_UP; // 产生递增递减波形
	   EPwm4Regs.TBCTL.bit.PHSEN = TB_DISABLE;        // Disable phase loading
	   EPwm4Regs.TBCTL.bit.SYNCOSEL = TB_SYNC_DISABLE; // Count up
	   EPwm4Regs.TBCTL.bit.HSPCLKDIV = 3;  //HSPCLKDIV=2*0Xxx 缺省值为1
	   EPwm4Regs.TBCTL.bit.CLKDIV = 7;     // CLKDIV=2^0Xxx 缺省值为1
	   ...
	   // Set actions
	   EPwm4Regs.AQCTLA.bit.ZRO= AQ_SET;          //通过改变CAU和ZRO的数值调整整AB两路的电平
	   EPwm4Regs.AQCTLA.bit.CAU= AQ_CLEAR;            //调整AB两路的电平
